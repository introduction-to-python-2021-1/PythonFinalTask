...
class TestRssReader(unittest.TestCase):
    ...
    def test_negative_limit(self):
        ...
        with self.assertRaises(SystemExit):
            with self.assertLogs('root', level='ERROR') as cm:
                main(argv)
        self.assertIn('ERROR:root: The limit argument must be greater than zero (-1 was passed)', cm.output)
    ...
    def test_zero_limit(self):
        ...
        with self.assertRaises(SystemExit):
            with self.assertLogs('root', level='ERROR') as cm:
                main(argv)
        self.assertIn('ERROR:root: The limit argument must be greater than zero (0 was passed)', cm.output)
