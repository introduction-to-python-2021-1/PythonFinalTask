...
class FeedContainer:
    ...
    def __get_feed_info(self):
        ...
        self.feed_title = self.__get_feed_title()
        self.feed_description = self.__get_feed_description()
        self.feed_link = self.__get_feed_link()
        self.feed_date = self.__get_feed_date()
        self.feed_copyright = self.__get_feed_copyright()
        self.__get_items()
        logger.info(
            f"Class {self.__class__.__name__}: data collection is done.")

    def __get_feed_title(self):
        return self.root.findtext("channel/title")

    def __get_feed_date(self):
        return self.root.findtext("channel/pubDate")

    def __get_feed_link(self):
        return self.root.findtext("channel/link")

    def __get_feed_description(self):
        return self.root.findtext("channel/description")

    def __get_feed_copyright(self):
        return self.root.findtext("channel/copyright")
